...
export function TagManager({
  ...
  function addTag(name: string) {
    ...
    startTransition(async () => {
      try {
        const tag = await assignTagToDrive(driveId, trimmed);
        ...
        setError(err instanceof Error ? err.message : t("tagManager.errorAdding"));
      }
    });
  }
  ...
  function removeTag(tag: TagLite) {
    ...
    startTransition(async () => {
      try {
        await removeTagFromDrive(driveId, tag.id);
      } catch (err) {
        setError(err instanceof Error ? err.message : t("tagManager.errorRemoving"));
        setAssigned((prev) =>
          prev.some((t) => t.id === tag.id) ? prev : [...prev, tag],
        );
      }
    ...
    <div className="flex flex-col gap-3">
        ...
        {assigned.map((tag) => (
          ...
          >
            ...
            <button
              ...
              aria-label={t("tagManager.removeAriaLabel", { name: tag.name })}
      ...
      <div className="relative">
        <input
          ...
          aria-label={t("tagManager.addAriaLabel")}
          disabled={pending}
